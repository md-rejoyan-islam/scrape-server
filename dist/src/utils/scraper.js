...
export async function scrapePage(options) {
    const { url, waitFor = 3000, extractors = [], fullHtml = false, screenshot = false, } = options;
    const startTime = Date.now();
    process.env.CHROME_PATH = puppeteer.executablePath();
    const { connect } = require("puppeteer-real-browser");
    const proxyStr = process.env.PROXY_URL;
    let customProxy = undefined;
    if (proxyStr) {
        try {
            ...
            console.log(`[scraper] Connecting via Proxy: ${customProxy.host}:${customProxy.port}`);
        }
        catch (e) {
...
function buildMetadata($, pageUrl, responseHeaders) {
    ...
    $("meta").each((_, el) => {
        ...
        if (element.attr("name"))
            attrs.name = element.attr("name");
        if (element.attr("property"))
            attrs.property = element.attr("property");
        if (element.attr("content"))
            attrs.content = element.attr("content");
        if (element.attr("http-equiv"))
            attrs.httpEquiv = element.attr("http-equiv");
        if (element.attr("charset"))
            attrs.charset = element.attr("charset");
...
function extractPrices($) {
    ...
    priceSelectors.forEach((selector) => {
        $(selector).each((_, el) => {
            ...
            const priceMatch = text.match(/[$€£₺₹]?\s*[\d,.]+\s*(?:TL|USD|EUR|GBP|₺|TRY)?/gi);
            if (priceMatch) {
...
// ─── BUILD PRODUCT DATA ─────────────────────────────────────
function buildProductData($, metadata, pageUrl) {
    ...
    if (!productWeight && productTitle) {
        const weightMatch = productTitle.match(/(\d+[\.,]?\d*)\s*(ml|l|g|kg|oz|fl\s*oz|lb|lbs|cl|mg|mm|cm|m)\b/i);
        if (weightMatch) {
    ...
    let currency = mainOffer?.priceCurrency ||
        getProductMicrodata(microdataItems, "priceCurrency") ||
        null;
    let sellPrice = parsePrice(mainOffer?.price ??
        mainOffer?.lowPrice ??
    ...
    if (sellPrice === null) {
        ...
        if (pagePrices.sell !== null) {
            ...
            if (!currency)
                currency = pagePrices.currency;
        }
    }
    const priceTry = currency || sellPrice !== null
        ? {
    ...
    if (!productFeatures) {
        ...
        if (feats.length > 0)
            productFeatures = feats.slice(0, 10).join(", ");
    }
    // ── Variants ──────────────────────────────────────────────
    const variants = [];
    ...
    if (hasVariantSource?.hasVariant) {
        ...
        for (const v of variantList) {
            variants.push(buildVariantFromLd(v, productSku, productWeight, productImagelinks, productInStock, pageUrl));
        }
    }
    ...
    if (variants.length === 0 && productLd?.model) {
        ...
        for (const m of models) {
            if (m && typeof m === "object") {
                variants.push(buildVariantFromLd(m, productSku, productWeight, productImagelinks, productInStock, pageUrl));
            }
        }
    }
    // Strategy 3: offers contain itemOffered (each offer wraps a variant product)
    if (variants.length === 0) {
        const offersWithItem = offers.filter((o) => o.itemOffered);
        if (offersWithItem.length > 0) {
            for (const offer of offersWithItem) {
                const item = offer.itemOffered;
                const v = { ...item, offers: offer };
                variants.push(buildVariantFromLd(v, productSku, productWeight, productImagelinks, productInStock, pageUrl));
            }
        }
    }
    ...
    if (variants.length === 0 && offers.length > 1) {
        for (const offer of offers) {
            variants.push({
                ...
                variant_weight: extractWeightFromText(offer.name || "") || productWeight,
                variant_size: extractVariantProp(offer, "size"),
                variant_image: resolveVariantImage(offer, pageUrl) || productImagelinks[0] || null,
    ...
    if (variants.length === 0 && allProductLds.length > 1) {
        ...
        const productBlocks = allProductLds.filter((p) => p["@type"] === "Product" || p["@type"] === "IndividualProduct");
        if (productBlocks.length > 1) {
            for (const p of productBlocks) {
                variants.push(buildVariantFromLd(p, productSku, productWeight, productImagelinks, productInStock, pageUrl));
            }
        }
    }
    ...
    if (variants.length === 0) {
        variants.push({
            ...
            variant_compare_at_price_try: regularPrice !== sellPrice ? regularPrice : null,
...
function isProductType(type) {
    ...
    if (Array.isArray(type))
        return type.some((t) => PRODUCT_TYPES.has(t));
    return false;
}
/** Recursively collect ALL JSON-LD objects that are Product-like */
function collectJsonLdProducts(obj, results) {
    if (!obj || typeof obj !== "object")
        return;
    if (Array.isArray(obj)) {
        for (const item of obj)
            collectJsonLdProducts(item, results);
        return;
    }
...
function buildVariantFromLd(v, fallbackSku, fallbackWeight, fallbackImages, fallbackInStock, pageUrl) {
    const vOffers = normalizeOffers(v.offers);
    const vOffer = vOffers[0] || null;
    // Determine option name/value from variant properties
    let optionName = v.name ? "Title" : null;
    let optionValue = v.name || null;
    ...
    return {
        ...
        variant_sku: v.sku ||
            ...
            fallbackSku ||
            null,
        variant_weight: extractWeightFromText(v.name || "") ||
            (v.weight
...
function resolveVariantImage(obj, pageUrl) {
    const img = obj.image;
    if (!img)
        return null;
    ...
    if (Array.isArray(img)) {
        ...
        if (first?.url)
            return resolveUrl(first.url, pageUrl);
        if (first?.contentUrl)
            return resolveUrl(first.contentUrl, pageUrl);
    }
    if (img.url)
        return resolveUrl(img.url, pageUrl);
    if (img.contentUrl)
        return resolveUrl(img.contentUrl, pageUrl);
    return null;
}
/** Extract a specific variant property (size, color, etc.) */
function extractVariantProp(obj, prop) {
    // Direct property
    if (obj[prop]) {
    ...
    if (obj.additionalProperty) {
        const props = Array.isArray(obj.additionalProperty)
            ...
            : [obj.additionalProperty];
        const match = props.find((p) => p.name?.toLowerCase() === prop.toLowerCase());
        if (match)
...
function parseAvailability(availability, fallback) {
    if (!availability)
        return fallback;
...
function extractWeightFromText(text) {
    if (!text)
        return null;
    const m = text.match(/(\d+[\.,]?\d*)\s*(ml|l|g|kg|oz|fl\s*oz|lb|lbs|cl|mg|mm|cm|m)\b/i);
    return m ? `${m[1]} ${m[2]}` : null;
}
/** Get a property from microdata items with Product type */
function getProductMicrodata(items, prop) {
    for (const item of items) {
        if (item.itemtype.toLowerCase().includes("product") ||
...
function extractVisiblePrices($) {
    ...
    const priceText = $('[itemprop="price"], [class*="price"]')
        ...
        .trim();
    if (priceText) {
        if (priceText.includes("₺") || priceText.includes("TL"))
            currency = "TRY";
...
function isExternalLink(href, base) {
    try {
